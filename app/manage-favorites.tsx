  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <ThemedText style={styles.hint}>点击星标添加或取消收藏，可在「我的」页快速打开。</ThemedText>
        }
        renderItem={({ item }) => {
          const on = favs.has(item.href);
          return (
            <TouchableOpacity style={styles.row} onPress={() => void onToggle(item.href)} activeOpacity={0.7}>
              <MaterialIcons name={item.icon as 'apps'} size={26} color="#007AFF" />
              <ThemedText style={styles.title}>{item.title}</ThemedText>
              <MaterialIcons name={on ? 'star' : 'star-border'} size={28} color={on ? '#FF9500' : '#C7C7CC'} />
            </TouchableOpacity>
          );
        }}
      />
