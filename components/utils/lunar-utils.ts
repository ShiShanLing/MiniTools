/**
 * 简易农历算法工具类 (Simplified Lunar Calendar Utility)
 * 移植自原 Angular 项目，支持 1900-2100
 */
export class LunarUtils {
  ...
  private static readonly MONTH_STR = ['正', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊'];
  ...
  public static getLunar(date: Date): any {
    ...
    if (temp < 0) {
      temp += isLeap ? this.leapDays(year) : this.monthDays(year, --i);
    }
    ...
    return {
      ...
      day,
      isLeap,
      monthStr: (isLeap ? '闰' : '') + this.MONTH_STR[month - 1] + '月',
      dayStr: this.getLunarDayStr(day)
    };
  }
  ...
  private static getLunarDayStr(day: number): string {
    ...
    if (day === 20) return '二十';
    if (day === 30) return '三十';
    const n = Math.floor(day / 10);
    const d = day % 10;
    return this.DAY_STR[n] + this.NUMBER_STR[d - 1];
  }
}
